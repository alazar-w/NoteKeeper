...
import static com.example.hinote.NoteKeeperDatabaseContract.*;
...
//            ourInstance.initializeExampleNotes();
    ...
    //to connect to the database we need noteKeeperOpenHelper class
    ...
    private static void loadCoursesFromDatabase(Cursor cursor) {
        ...
        //we have now the position of course title and course id. so now we want to load these courses into the lists
        //contained in our DataManager singleton,so in order to access that list,the first thing we need to do is get a reference to the singleton.
        //so the method getInstance returns us back a reference to the instance and we store it in our local there  dm. and then we clear out any courses that might already have been loaded(dm.mCourses.clear,we r making sure
        // we start with fresh clean list each time we load it up)
        DataManager dm = getInstance();
        dm.mCourses.clear();
        //we call cursor.moveToNext here because our cursor starts before the first item so this call move the cursor the the first(to index 0)
        while (cursor.moveToNext()){
            String courseId = cursor.getString(courseIdPos);
            String courseTitle = cursor.getString(courseTitlePos);
            //we don't need the 3rd parameter that why we put null
